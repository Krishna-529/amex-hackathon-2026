...
| Cancellation detection | Two lanes: a tier-gated status poller (`server/engine/statusPoller.ts`, hard-capped well under AviationStack's 100/month) and member reports (`server/engine/memberReports.ts`) that act for the reporter immediately and for everyone else only once corroborated |
