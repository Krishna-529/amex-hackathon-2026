...
import { localTime, isInternational } from '@/server/airportDirectory';
...
export async function GET(req: NextRequest) {
  ...
  const body: AltsResponse = {
    alts: offers.map((o) => toAlt(o, origin, destination, profile.preferences.cabinEntitlement, profile.preferences.perTransactionCap)),
    offers,
 ...
 * system of record for what cabin the member may be moved into and what a single
 * transaction may cost. A candidate outside either is surfaced and marked, never
 * silently dropped: the member is allowed to see what was ruled out.
 */
function toAlt(
  o: Offer,
  from: string,
  ...
  // The cap is set in the card's billing currency. We do not hold FX rates, so a
  // fare quoted in another currency cannot be compared to it — and guessing
  // would be worse than admitting it. Those go to the member rather than being
  // silently passed or silently blocked.
  ...
  const ok = !overCabin && !overCap && !needsConversion;
  ...
  const why = overCabin
    ...
    : overCap
      ...
      : needsConversion
        ? `Priced in ${o.price.currency}, and your cap is set in ${cap.currency} — we will not approve this automatically without converting it`
        : o.live
