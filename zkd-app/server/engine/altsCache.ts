 ...
 */
function watcherCount(): number {
  const seen = store.listFlights().filter((f) => f.altsAsOf !== undefined).length;
 ...
 */
export function altsRefreshPlan(flight: Flight): RefreshInterval {
  ...
  const disrupted = store.getDisruptionEvent(flight.id) !== undefined;
  ...
  return refreshIntervalFor({
    ...
    hasOpenConsentWindow: store
      .getRecoveryTasksForFlight(flight.id)
...
export function isAltsStale(flight: Flight): boolean {
  ...
  return Date.now() - flight.altsAsOf > altsRefreshPlan(flight).ms;
}
 ...
 */
export function invalidateAlts(flightId: string, reason: string): void {
 ...
 */
export function refreshAltsNow(flightId: string, reason: string): Promise<void> {
  invalidateAlts(flightId, reason);
...
async function compute(flightId: string, lane: Lane): Promise<void> {
  ...
  flight.candidates.alts = mergePinned(flightId, flight.candidates.alts, fresh);
 ...
 */
function mergePinned(flightId: string, current: Alt[], fresh: Alt[]): Alt[] {
  ...
  for (const p of store.listPassengers()) {
    const pre = store.getPreAuth(flightId, p.id);
...
export function refreshAltsIfStale(flight: Flight): void {
  ...
  void refreshAlts(flight.id).catch((err) => {
