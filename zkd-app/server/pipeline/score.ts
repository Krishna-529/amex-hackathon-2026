...
import { weightsFor, STRATEGY_LABEL, type Criterion, type Weights } from '../preferences/presets';
...
function score(
  ...
  //
  // A gentler score than an earlier draft's 0.6 matters: too harsh and a
  // member who asked for `lowest_cost` gets handed a paid fare that arrived
  // twenty minutes earlier, because the reliability gap alone outvoted being
  // free — overriding a stated preference, which this guard must not do —
  // and it also makes Guard 3 unreachable, since reliability always decides
  // first. Guard 2 should nudge; Guard 3 is what handles the genuinely
  // lopsided cases.
  ...
  return {
    ...
    score: {
      ...
      notes,
      // Placeholder — finalise() below overwrites this with the real
      // strongest-contributing criterion once every candidate is scored and
      // the early-arrival override (if any) has been applied. Never read
      // before finalise() runs.
      leadingCriterion: 'reliability',
    },
    why: '',
  };
}
...
function finalise(scored: ScoredAlt[], ctx: ScoreContext): ScoredAlt[] {
  return scored.map((s) => {
    const { text, leadingCriterion } = explain(s, ctx);
    return { ...s, why: text, score: { ...s.score, leadingCriterion } };
  });
}
 ...
 * deserves the true one. Also returns which criterion led, so callers (the
 * OptionReason surfaced in the UI) don't have to recompute this.
 */
function explain(s: ScoredAlt, ctx: ScoreContext): { text: string; leadingCriterion: Criterion } {
  ...
  const leadingCriterion = contributions[0].k;
  return {
    text: `You asked us to optimise for ${STRATEGY_LABEL[ctx.rules.strategy]}, so we picked this because ${lead[leadingCriterion]}. ${detail}.`,
    leadingCriterion,
  };
}
