 ...
 *
 * `detectDisruption` awaits this module's entry point as part of building the
 * response body for POST /api/disruptions. An exception escaping the pipeline
 * would take the trigger endpoint down with it, so every entry point below is
 * either `void`-called with an internal catch, or returns a value on failure.
 * This is the highest-severity failure mode in the design and it is handled
...
export async function execute(task: RecoveryTask, resolution: DisruptionResolution): Promise<void> {
  ...
  // lookup happens here instead of inside it.
