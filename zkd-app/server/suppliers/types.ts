...
/**
 * `kiwi`, `skyscanner` and `travelfusion` join the original three, and
 * `sandbox` is the write-capable adapter the reissue/rollback paths run
 * against (registered only under `ZKD_SANDBOX=1`, see ./index.ts). Nothing
 * downstream distinguishes the read-only sources — they satisfy the same
 * contract, which is the point: swapping a sandbox for an enterprise source
 * later is a registry edit in ./index.ts, not a rewrite of anything that
 * consumes offers.
 */
